	...
	<view>
		<view class="content">
			...
			<view class="row">
				...
				<view class="input" @tap="chooseCity" v-if="this.cityStr">{{ this.cityStr}}</view>
				<view class="input" @tap="chooseCity" v-else>{{ region.label}}</view>
			</view>
			<view class="row">
...
import { addressSave, addressDel, addressUpdate } from '../../../../api/mine/address.js';
export default {
	components: {
		mpvueCityPicker
	},
	...
	methods: {
		...
		save() {
			...
			// })
			var cityCodeArr = [];
			cityCodeArr[0] = data.address.region.cityCode;
			for (var i = 0; i < data.address.region.value.length; i++) {
				cityCodeArr.push(data.address.region.value[i]);
			}
			var params = {
				userId: this.$store.getters.userId,
				tag: '家',
				cityCode: cityCodeArr.join(),
				cityStr: data.address.region.label,
				cityBak: data.address.detailed,
				tel: data.tel,
				name: data.name,
				isUse: data.isDefault ? '1' : '0'
			}
			var api = null;
			if(this.id){
					  api = addressUpdate;
		...
		if (e.type == 'edit') {
			uni.getStorage({
				...
				success: e => {
					...
					this.tel = e.data.tel;
					this.detailed = e.data.cityBak;
					this.isDefault = e.data.isUse == '1' ? true : false;
					this.cityPickerValue = e.data.cityCode
						.split(',')
						.splice(1, 3)
						.map(Number);
					this.cityStr = e.data.cityStr;
				}
			});
		}
