...
<script>
	var _this;
	import {doLogin} from '../../../api/mine/user.js'
	import {addressList} from "../../../api/mine/address.js"
	...
	export default {
		...
		methods: {
		   ...
		   async startLogin(){
				...
				if(res.code == "S"){
					var addressRes = await addressList({userId:res.data.id})
					var addressUse = addressRes.data.filter(item =>item.isUse == "1")
